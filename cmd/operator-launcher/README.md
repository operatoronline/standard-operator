...
A standalone launcher for Operator, providing visual JSON editing and OAuth provider authentication management.
...
# Run with default config path (~/.operator/config.json)
./operator-launcher
...
```
Usage: operator-config [options] [config.json]

Arguments:
  config.json    Path to the configuration file (default: ~/.operator/config.json)
...
{
  "config": { ... },
  "path": "/Users/xiao/.operator/config.json"
}
```
...
After authentication, Google redirects to `GET /auth/callback`, which saves the credentials and redirects back to the operator-config UI.
...
Gets the running status of the `operator gateway` process.
...
Starts the `operator gateway` process in the background.
...
Stops the running `operator gateway` process.
...
```bash
go test -v ./cmd/operator-launcher/
```
