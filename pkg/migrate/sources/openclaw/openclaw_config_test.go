...
func TestLoadOpenClawConfig(t *testing.T) {
	...
	if workspace != "~/.operator/workspace" {
		t.Errorf("expected workspace '~/.operator/workspace', got '%s'", workspace)
	}
...
func TestConvertToOperator(t *testing.T) {
	...
	operatorCfg, warnings, err := cfg.ConvertToOperator("")
	if err != nil {
		t.Fatalf("failed to convert config: %v", err)
	}

	if operatorCfg.Agents.Defaults.ModelName != "claude-sonnet-4-20250514" {
		t.Errorf("expected model 'claude-sonnet-4-20250514', got '%s'", operatorCfg.Agents.Defaults.ModelName)
	}
	if operatorCfg.Agents.Defaults.Workspace != "~/.operator/workspace" {
		t.Errorf("expected workspace '~/.operator/workspace', got '%s'", operatorCfg.Agents.Defaults.Workspace)
	}

	if len(operatorCfg.Agents.List) != 2 {
		t.Errorf("expected 2 agents, got %d", len(operatorCfg.Agents.List))
	}
	if operatorCfg.Agents.List[0].ID != "main" {
		t.Errorf("expected first agent id 'main', got '%s'", operatorCfg.Agents.List[0].ID)
	}
	if operatorCfg.Agents.List[1].Skills == nil || len(operatorCfg.Agents.List[1].Skills) != 2 {
		t.Errorf("expected 2 skills for assistant agent")
	}

	if !operatorCfg.Channels.Telegram.Enabled {
		t.Error("telegram should be enabled")
	}
	if operatorCfg.Channels.Telegram.Token != "test-token" {
		t.Errorf("expected telegram token 'test-token', got '%s'", operatorCfg.Channels.Telegram.Token)
	}

	if operatorCfg.Channels.WhatsApp.BridgeURL != "http://localhost:3000" {
		t.Errorf("expected whatsapp bridge URL 'http://localhost:3000', got '%s'", operatorCfg.Channels.WhatsApp.BridgeURL)
	}

	if operatorCfg.Channels.Feishu.AppID != "app-id" {
		t.Errorf("expected feishu app ID 'app-id', got '%s'", operatorCfg.Channels.Feishu.AppID)
	}

	if len(operatorCfg.ModelList) != 1 {
		t.Errorf("expected 1 model config (no models.json provided), got %d", len(operatorCfg.ModelList))
	}
...
func TestConvertToOperatorWithQQAndDingTalk(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	operatorCfg, _, err := cfg.ConvertToOperator("")
	if err != nil {
		t.Fatalf("failed to convert config: %v", err)
	}

	if !operatorCfg.Channels.QQ.Enabled {
		t.Error("qq should be enabled")
	}
	if operatorCfg.Channels.QQ.AppID != "qq-app-id" {
		t.Errorf("expected qq app ID 'qq-app-id', got '%s'", operatorCfg.Channels.QQ.AppID)
	}

	if !operatorCfg.Channels.DingTalk.Enabled {
		t.Error("dingtalk should be enabled")
	}
	if operatorCfg.Channels.DingTalk.ClientID != "ding-app-id" {
		t.Errorf("expected dingtalk client ID 'ding-app-id', got '%s'", operatorCfg.Channels.DingTalk.ClientID)
	}

	if !operatorCfg.Channels.MaixCam.Enabled {
		t.Error("maixcam should be enabled")
	}
	if operatorCfg.Channels.MaixCam.Host != "192.168.1.100" {
		t.Errorf("expected maixcam host '192.168.1.100', got '%s'", operatorCfg.Channels.MaixCam.Host)
	}
	if operatorCfg.Channels.MaixCam.Port != 9000 {
		t.Errorf("expected maixcam port 9000, got %d", operatorCfg.Channels.MaixCam.Port)
	}

	if !operatorCfg.Channels.Slack.Enabled {
		t.Error("slack should be enabled")
	}
	if operatorCfg.Channels.Slack.BotToken != "xoxb-test" {
		t.Errorf("expected slack bot token 'xoxb-test', got '%s'", operatorCfg.Channels.Slack.BotToken)
	}
	if operatorCfg.Channels.Slack.AppToken != "xapp-test" {
		t.Errorf("expected slack app token 'xapp-test', got '%s'", operatorCfg.Channels.Slack.AppToken)
	}
}
...
func TestToStandardConfig(t *testing.T) {
	...
	stdCfg := operatorCfg.ToStandardConfig()
	...
	if stdCfg.Agents.Defaults.Workspace != "~/.operator/workspace" {
		t.Errorf("expected workspace '~/.operator/workspace', got '%s'", stdCfg.Agents.Defaults.Workspace)
	}
