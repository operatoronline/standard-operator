...
func TestDefaultConfig_WorkspacePath_Default(t *testing.T) {
	...
	t.Setenv("OPERATOR_HOME", "")
	// Set a known home for consistent test results
	t.Setenv("HOME", "/tmp/home")

	cfg := DefaultConfig()
	want := filepath.Join("/tmp/home", ".operator", "workspace")
...
func TestDefaultConfig_WorkspacePath_WithOperatorHome(t *testing.T) {
	...
	cfg := DefaultConfig()
	want := "/custom/operator/home/workspace"

	if cfg.Agents.Defaults.Workspace != want {
		t.Errorf("Workspace path with OPERATOR_HOME = %q, want %q", cfg.Agents.Defaults.Workspace, want)
	}
}
