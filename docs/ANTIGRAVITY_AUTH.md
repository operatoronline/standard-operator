...
**Antigravity** (Google Cloud Code Assist) is a Google-backed AI model provider that offers access to models like Claude Opus 4.6 and Gemini through Google's Cloud infrastructure. This document provides a complete guide on how authentication works, how to fetch models, and how to implement a new provider in Operator.
...
10. [Creating a New Provider in Operator](#creating-a-new-provider-in-operator)
...
const CLIENT_SECRET = decode("REDACTED_CLIENT_SECRET_BASE64");
```
...
type ProviderAuthResult = {
  ...
  configPatch?: Partial<OperatorConfig>;
  defaultModel?: string;
  notes?: string[];
};
```
...
Auth profiles are stored in `~/.operator/auth.json`:
...
## Creating a New Provider in Operator

Operator providers are implemented as Go packages under `pkg/providers/`. To add a new provider:
...
If your provider requires OAuth or special authentication, add a case to `cmd/operator/cmd_auth.go`:
...
operator auth login --provider your-provider
...
operator agent -m "Hello" --model your-model
```
...
export OPERATOR_AGENTS_DEFAULTS_MODEL=your-model

# Override provider settings
export OPERATOR_MODEL_LIST='[{"model_name":"your-model","model":"your-provider/model-name","api_key":"..."}]'
```
...
### "Token expired"
- Refresh OAuth tokens: `operator auth login --provider antigravity`
...
- Check auth profile storage: `~/.operator/auth.json`
- Re-run `operator auth login --provider antigravity`
