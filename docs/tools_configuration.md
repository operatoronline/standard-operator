...
Operator's tools configuration is located in the `tools` field of `config.json`.
...
By default, Operator blocks the following dangerous commands:
...
All configuration options can be overridden via environment variables with the format `OPERATOR_TOOLS_<SECTION>_<KEY>`:

For example:

- `OPERATOR_TOOLS_WEB_BRAVE_ENABLED=true`
- `OPERATOR_TOOLS_EXEC_ENABLE_DENY_PATTERNS=false`
- `OPERATOR_TOOLS_CRON_EXEC_TIMEOUT_MINUTES=10`
- `OPERATOR_TOOLS_MCP_ENABLED=true`
